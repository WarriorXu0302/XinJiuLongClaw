...
function RetailWarehouseManage() {
  ...
  const filteredFlows = flows.filter(f => retailWhIds.has(f.warehouse_id));
  ...
  const outboundMutation = useMutation({
    ...
    onSuccess: () => {
      ...
      queryClient.invalidateQueries({ queryKey: ['retail-wh-flows'] });
  ...
  return (
    <>
      <Space style={{ marginBottom: 16, justifyContent: 'space-between', width: '100%' }}>
        <div>
          <Title level={4} style={{ margin: 0 }}>零售仓库管理</Title>
          <Text type="secondary">零售仓库的出入库管理</Text>
        </div>
      ...
      <Modal title="零售仓库 — 直接出库" open={modalOpen}
        ...
        <Form form={form} layout="vertical">
          <Form.Item name="warehouse_id" label="出库仓库" rules={[{ required: true }]}>
            <Select placeholder="选择零售仓库"
              options={retailWarehouses.map(w => ({ value: w.id, label: w.name }))} />
          </Form.Item>
...
export default RetailWarehouseManage;
