...
**白酒扫码铁律**：每瓶都有厂家防伪码。采购收货 / 业务员出库 / 仓库调拨 / 门店零售出库 —— 全部必须扫码过户，绝不允许按数量散装。

**门店零售（桥 B12）**：4 家专卖店 = `warehouses.warehouse_type='store'` 的仓。店员（employees.position='cashier' + assigned_store_id）小程序收银：扫码出库 + 输入售价（须在 products.min/max_sale_price 区间内）+ 选付款方式（cash/wechat/alipay/card，禁赊账）+ 提交。提成 = (售价 - 成本) × `retail_commission_rates.rate_on_profit`（每员工×每商品一个点）→ 一条 Commission pending → 月结工资单纳入。不走审批。
